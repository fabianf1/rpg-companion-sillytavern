 ...
 */
export async function ensureHtmlCleaningRegex(
	...
	try {
		...
		if (!st_extension_settings || typeof st_extension_settings !== "object") {
			console.warn(
				"[RPG Companion] Invalid extension_settings object, skipping HTML regex import",
			);
			return;
		}
		...
		if (!Array.isArray(existingScripts)) {
			console.warn(
				"[RPG Companion] extension_settings.regex is not an array, resetting to empty array",
			);
			st_extension_settings.regex = [];
		}

		const alreadyExists = existingScripts.some(
			(script) =>
				script &&
		...
		if (typeof saveSettingsDebounced === "function") {
			...
			console.warn(
				"[RPG Companion] saveSettingsDebounced is not a function, cannot save HTML regex",
			);
		}
		...
		console.error(
			"[RPG Companion] Failed to import HTML cleaning regex:",
			error,
		);
 ...
 * @param {Function} saveSettingsDebounced - Function to save settings
 */
export async function ensureTrackerCleaningRegex(
	st_extension_settings,
	saveSettingsDebounced,
) {
	try {
		...
		if (!st_extension_settings || typeof st_extension_settings !== "object") {
			console.warn(
				"[RPG Companion] Invalid extension_settings object, skipping tracker cleaning regex import",
			);
			return;
		}
		...
		if (typeof saveSettingsDebounced === "function") {
			...
			console.warn(
				"[RPG Companion] saveSettingsDebounced is not a function, cannot save tracker cleaning regex",
			);
		}
		...
		console.error(
			"[RPG Companion] Failed to import tracker cleaning regex:",
			error,
		);
		console.error("[RPG Companion] Error details:", error.message, error.stack);
		// Don't throw - this is a nice-to-have feature
