 ...
 */
export async function ensureJsonCleaningRegex(
	...
	try {
		...
		if (!st_extension_settings || typeof st_extension_settings !== "object") {
			console.warn(
				"[RPG Companion] Invalid extension_settings object, skipping JSON cleaning regex",
			);
			return;
		}
		...
		if (existingScript) {
			...
			if (needsSave && typeof saveSettingsDebounced === "function") {
				...
				console.log(
					"[RPG Companion] ✅ Updated JSON cleaning regex to v3.2.3 settings.",
				);
			} else {
		...
		if (typeof saveSettingsDebounced === "function") {
			...
			console.warn(
				"[RPG Companion] saveSettingsDebounced is not a function, cannot save JSON cleaning regex",
			);
		}
	} catch (error) {
		console.error(
			"[RPG Companion] JSON Cleaning Regex failed to properly initialize!",
		);
 ...
 * @param {Function} saveSettingsDebounced - Function to save settings
 */
export function removeJsonCleaningRegex(
	st_extension_settings,
	saveSettingsDebounced,
) {
	try {
		if (
			!st_extension_settings?.regex ||
			!Array.isArray(st_extension_settings.regex)
		) {
			return;
		}
		...
		console.error(
			"[RPG Companion] Failed to remove JSON cleaning regex:",
			error,
		);
	}
}
