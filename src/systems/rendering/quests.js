...
function renderQuestItem(field, quest, index) {
	const { questTitle, questDate, questLocation, isCompleted } =
		extractQuestData(quest);
	...
	// Build lock path
	const lockPath =
		field === "main" ? "quests.main" : `quests.optional[${index}]`;
...
function renderQuestForm(field, action, quest) {
	...
	const dateId =
		field === "main"
			? `rpg-${prefix}-quest-date`
			: `rpg-${prefix}-quest-date-${field}`;
	const locationId =
		field === "main"
			? `rpg-${prefix}-quest-location`
			: `rpg-${prefix}-quest-location-${field}`;
	const completedId =
		field === "main"
			? `rpg-${prefix}-quest-completed`
			: `rpg-${prefix}-quest-completed-${field}`;
...
export function renderOptionalQuestsView(optionalQuests) {
	...
	if (quests.length === 0) {
		...
		questsHtml = quests
			.map((quest, index) => renderQuestItem("optional", quest, index))
			.join("");
	}
...
export function renderQuests() {
	...
	if (!trackerData || !trackerData.quests) {
		$questsContainer.html(
			'<div class="rpg-inventory-empty">No quests generated yet</div>',
		);
		return;
	}
	...
	while (
		mainQuest &&
		typeof mainQuest === "object" &&
		mainQuest.value !== undefined
	) {
...
function attachQuestEventHandlers() {
	...
	// Cancel add quest
	$questsContainer
		.find('[data-action="cancel-add-quest"]')
		.on("click", function () {
			const field = $(this).data("field");
			...
			$(
				`#rpg-new-quest-location${field === "optional" ? "-" + field : ""}`,
			).val("");
			$(
				`#rpg-new-quest-completed${field === "optional" ? "-" + field : ""}`,
			).prop("checked", false);
		});

	// Save add quest
	$questsContainer
		.find('[data-action="save-add-quest"]')
		.on("click", function () {
			const field = $(this).data("field");
	...
	$questsContainer.find('[data-action="edit-quest"]').on("click", function () {
		...
		const quests = getTrackerDataForContext("userStats")?.quests || {
			main: null,
			optional: [],
		};
	...
	// Cancel edit quest
	$questsContainer
		.find('[data-action="cancel-edit-quest"]')
		.on("click", function () {
			const field = $(this).data("field");
	...
	// Save edit quest
	$questsContainer
		.find('[data-action="save-edit-quest"]')
		.on("click", function () {
			const field = $(this).data("field");
			const index =
				field === "optional"
					? $(`#rpg-edit-quest-form-${field}`).data("edit-index")
					: 0;
	...
	$questsContainer
		.find(".rpg-quest-completed-checkbox")
		.on("change", function () {
			const field = $(this).data("field");
			const index = $(this).data("index");
	...
	$questsContainer
		.on("mouseenter", ".rpg-quest-item", function () {
