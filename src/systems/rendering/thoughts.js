...
export function renderThoughts({ preserveScroll = false } = {}) {
	...
	debugLog(
		"[RPG Thoughts] ==================== RENDERING PRESENT CHARACTERS ====================",
	);
	debugLog(
		"[RPG Thoughts] showCharacterThoughts setting:",
	...
	debugLog(
		"[RPG Thoughts] Data length:",
		characterThoughtsData.length + " chars",
	);
	debugLog(
		"[RPG Thoughts] Enabled custom fields:",
		enabledFields.map((f) => f.name),
	);
	debugLog(
		"[RPG Thoughts] Enabled character stats:",
		enabledCharStats.map((s) => s.name),
	);
	...
	debugLog(
		"[RPG Thoughts] ==================== BUILDING HTML ====================",
	);
	debugLog(
		"[RPG Thoughts] Starting HTML generation for",
		presentCharacters.length + " characters",
	);
	...
	if (presentCharacters.length === 0) {
		...
		for (const char of presentCharacters) {
			...
			try {
				debugLog(
					`[RPG Thoughts] Building HTML for character ${characterIndex}/${presentCharacters.length}:`,
					char.name,
				);
				...
				if (characterPortrait === FALLBACK_AVATAR_DATA_URI && selected_group) {
					...
					try {
						...
						debugLog(
							"[RPG Thoughts] Group members count:",
						...
						debugLog(
							"[RPG Thoughts] Error checking group members:",
							groupError.message,
						);
					}
				}
				...
				if (
					characterPortrait === FALLBACK_AVATAR_DATA_URI &&
					characters &&
					characters.length > 0
				) {
				...
				debugLog(
					`[RPG Thoughts] Final avatar for ${char.name}:`,
				...
				debugLog(
					`[RPG Thoughts] ✗ ERROR building HTML for ${char.name}:`,
					charError.message,
				);
	...
	if (extensionSettings.enableAnimations) {
		setTimeout(
			() => $thoughtsContainer.removeClass("rpg-content-updating"),
			600,
		);
	}
	...
	if (isJSON) {
		...
		const updatedJSON = JSON.stringify(
			Array.isArray(parsedData)
				? charactersArray
				: { ...parsedData, characters: charactersArray },
			null,
			2,
		);
		updateMessageSwipeData("characterThoughts", updatedJSON);
...
export function updateCharacterField(characterName, field, value) {
	...
	if (isJSON) {
		const charactersArray = Array.isArray(parsedData)
			? parsedData
			: parsedData.characters || [];
		const charIndex = charactersArray.findIndex(
		...
		const isEditingThoughts =
			field === thoughtsFieldName || field === "thoughts";
	...
	if (characterFound) {
		...
		const thoughtsFieldName = presentCharsConfig?.thoughts?.name || "Thoughts";
		const isThoughtsField =
			field.toLowerCase() === "thoughts" || field === thoughtsFieldName;
		...
		if (isStatField) {
			...
			if (statsLineExists) {
				...
				for (let i = characterStartIndex; i < characterEndIndex; i++) {
					const line = lines[i].trim();
					const thoughtsFieldName =
						presentCharsConfig?.thoughts?.name || "Thoughts";
...
function attachDragHandlersToIcon($icon) {
	...
	$icon.on("click.thoughtIconDrag", (e) => {
	...
	$icon.on("touchmove.thoughtIconDrag", function (e) {
		...
		if (
			!isDragging &&
			(timeSinceStart > LONG_PRESS_DURATION || distance > DRAG_THRESHOLD)
		) {
			isDragging = true;
	...
	$(document).on("mousemove.thoughtIconDrag", (e) => {
	...
	$(document).on("mouseup.thoughtIconDrag", (e) => {
		if (!mouseDown) return;
...
export function createThoughtPanel($message, thoughtsArray) {
	...
	const updateBubblePosition = () => {
		...
		if (isRightPanel) {
			$thoughtPanel
				...
				.removeClass("rpg-thought-panel-right")
			...
			$thoughtPanel
				...
				.removeClass("rpg-thought-panel-left")
	...
	if (isMobileView) {
		...
		if (
			...
			if (savedTop < topBarHeight + 50) {
				...
				setTimeout(() => {
					...
					const defaultTop =
						topBarHeight + (viewportHeight - topBarHeight) / 2 - 22;
			...
			setTimeout(() => {
				...
				const defaultTop =
					topBarHeight + (viewportHeight - topBarHeight) / 2 - 22; // 22 = half of icon height
		...
		$thoughtIcon
			.addClass("rpg-force-hide")
			.removeClass("rpg-collapsed-desktop");
	}
	...
	$thoughtPanel.find(".rpg-thought-close").on("click touchend", (e) => {
		e.preventDefault();
		e.stopPropagation();
		...
		if (isMobileView) {
			...
			$icon.css({
				...
				right: isRightPanel
					? "auto"
					: `${window.innerWidth - panelRect.right}px`,
			});
	...
	$(document).on("click.thoughtPanel", (e) => {
		// Only hide on click outside in mobile view
		if (window.innerWidth <= 1000) {
			if (
				!$(e.target).closest("#rpg-thought-panel, #rpg-thought-icon").length
			) {
