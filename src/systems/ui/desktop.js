...
export function updateStripWidgets() {
	...
	if (!widgetSettings || !widgetSettings.enabled) {
		...
		$container
			.find(".rpg-strip-widget")
			.removeClass("rpg-strip-widget-visible");
		return;
	}
	...
	if (widgetSettings.clock?.enabled && infoData?.time) {
		...
		if (timeStr) {
			...
			$clockWidget
				.find(".rpg-strip-clock-hour")
				.css("transform", `rotate(${hourAngle}deg)`);
			$clockWidget
				.find(".rpg-strip-clock-minute")
	...
	if (widgetSettings.date?.enabled && infoData?.date?.value) {
		...
		const displayDate =
			dateVal.length > 20 ? dateVal.substring(0, 18) + "…" : dateVal;
	...
	if (widgetSettings.stats?.enabled) {
		...
		if (userStatsData) {
			try {
				...
				console.warn(
					"[RPG Strip Widgets] Failed to parse tracker userStats:",
					e,
				);
			}
		}
		...
		if (allStats.length > 0) {
			...
			allStats.forEach((stat) => {
				...
				if (configuredStats.length > 0) {
					...
					if (!enabledStatMap.has(statId) && !enabledStatMap.has(statName))
						return;
				}
				...
				const $item =
					$(`<div class="rpg-strip-stat-item" title="${stat.name}: ${value}">
	...
	if (widgetSettings.attributes?.enabled) {
		...
		if (showRPGAttributes && extensionSettings.classicStats) {
			...
			Object.entries(attrs).forEach(([key, value]) => {
				...
				if (
					enabledAttrIds.length > 0 &&
					!enabledAttrIds.includes(key.toLowerCase())
				) {
					return;
				}

				const $item =
					$(`<div class="rpg-strip-attr-item" title="${key.toUpperCase()}: ${value}">
...
export function setupDesktopTabs() {
	...
	const $statusTab = $(
		'<div class="rpg-tab-content active" data-tab-content="status"></div>',
	);
	const $appearanceTab = $(
		'<div class="rpg-tab-content" data-tab-content="appearance"></div>',
	);
	const $inventoryTab = $(
		'<div class="rpg-tab-content" data-tab-content="inventory"></div>',
	);
	const $questsTab = $(
		'<div class="rpg-tab-content" data-tab-content="quests"></div>',
	);
