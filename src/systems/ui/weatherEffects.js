...
export const WEATHER_PATTERNS_BY_LANGUAGE = {
	...
	ru: [
		...
		{
			id: "sunny",
			patterns: ["солнечно", "ясно", "ярко", "ясное утро", "ясный день"],
		},
...
export function getWeatherKeywordsForPrompt(language) {
	...
	for (const [_lang, patterns] of Object.entries(languagesToProcess)) {
...
function getCurrentWeather() {
	...
	try {
		const parsed =
			typeof infoBoxData === "object" ? infoBoxData : JSON.parse(infoBoxData);
		if (parsed?.weather) {
			// Return the condition text from the weather object (new: icon/condition)
			return parsed.weather.condition || parsed.weather.icon || null;
		}
	} catch (_e) {
		// Not JSON, try old text format
	}
...
function createSunrise(hour) {
	...
	const sunGlow = document.createElement("div");
	sunGlow.className =
		"rpg-weather-particle rpg-clear-sun-glow rpg-sunrise-glow";
	sunGlow.style.left = `${sunPos.left}vw`;
	sunGlow.style.top = `${sunPos.top}dvh`;
	container.appendChild(sunGlow);
	...
	for (let i = 0; i < 15; i++) {
		...
		star.className =
			"rpg-weather-particle rpg-night-star rpg-sunrise-fading-star";
...
function createSunset(hour) {
	...
	for (let i = 0; i < 20; i++) {
		const star = document.createElement("div");
		star.className =
			"rpg-weather-particle rpg-night-star rpg-sunset-emerging-star";
		star.style.left = `${Math.random() * 100}vw`;
	...
	for (let i = 0; i < 12; i++) {
		const particle = document.createElement("div");
		particle.className =
			"rpg-weather-particle rpg-clear-dust-mote rpg-sunset-dust";
		particle.style.left = `${Math.random() * 100}vw`;
		particle.style.top = `${Math.random() * 100}dvh`;
		particle.style.animationDelay = `${Math.random() * 15}s`;
		particle.style.animationDuration = `${12 + Math.random() * 8}s`;
...
function createNighttime(hour) {
	...
	for (let i = 0; i < 8; i++) {
		...
		brightStar.className =
			"rpg-weather-particle rpg-night-star rpg-night-star-bright";
