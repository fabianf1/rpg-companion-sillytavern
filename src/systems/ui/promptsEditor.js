...
export function initPromptsEditor() {
	...
	// Save button
	$(document).on("click", "#rpg-prompts-save", () => {
		savePrompts();
		closePromptsEditor();
	...
	// Cancel button
	$(document).on("click", "#rpg-prompts-cancel", () => {
		closePromptsEditor();
	});

	// Close X button
	$(document).on("click", "#rpg-close-prompts-editor", () => {
		closePromptsEditor();
	});

	// Restore All button
	$(document).on("click", "#rpg-prompts-restore-all", () => {
	...
	$(document).on("click", "#rpg-prompts-editor-popup", (e) => {
	...
	// Open button
	$(document).on("click", "#rpg-open-prompts-editor", () => {
		openPromptsEditor();
	});
}
...
function openPromptsEditor() {
	...
	$("#rpg-prompt-narrator").val(
		extensionSettings.customNarratorPrompt || DEFAULT_PROMPTS.narrator,
	);
	...
	$("#rpg-prompt-plot-natural").val(
		extensionSettings.customPlotNaturalPrompt || DEFAULT_PROMPTS.plotNatural,
	);
...
function savePrompts() {
	extensionSettings.customNarratorPrompt = $("#rpg-prompt-narrator")
		.val()
		.trim();
	extensionSettings.customContextInstructionsPrompt = $(
		"#rpg-prompt-context-instructions",
	)
		.val()
		.trim();
	extensionSettings.customPlotRandomPrompt = $("#rpg-prompt-plot-random")
		.val()
		.trim();
	extensionSettings.customPlotNaturalPrompt = $("#rpg-prompt-plot-natural")
		.val()
		.trim();
	extensionSettings.customTrackerInstructionsPrompt = $(
		"#rpg-prompt-tracker-instructions",
	)
		.val()
		.trim();
	extensionSettings.customTrackerContinuationPrompt = $(
		"#rpg-prompt-tracker-continuation",
	)
		.val()
		.trim();
	extensionSettings.customCombatNarrativePrompt = $(
		"#rpg-prompt-combat-narrative",
	)
		.val()
		.trim();
...
function restorePromptToDefault(promptType) {
	...
	$(`#rpg-prompt-${promptType.replace(/([A-Z])/g, "-$1").toLowerCase()}`).val(
		defaultValue,
	);
