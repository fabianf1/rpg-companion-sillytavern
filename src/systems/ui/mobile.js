...
export function setupMobileToggle() {
	...
	if ($mobileToggle.length === 0) {
		...
		console.error(
			"[RPG Mobile] Cannot attach event handlers - button does not exist",
		);
	...
	$mobileToggle.on("touchmove", (e) => {
		...
		if (
			!isDragging &&
			(timeSinceStart > LONG_PRESS_DURATION || distance > MOVE_THRESHOLD)
		) {
			isDragging = true;
	...
	$(window).on("resize", () => {
		...
		if (!wasMobile && isMobile) {
			...
			$panel.removeClass(
				"rpg-position-right rpg-position-left rpg-position-top",
			);
			$("body").removeClass(
				"rpg-panel-position-right rpg-panel-position-left rpg-panel-position-top",
			);
...
export function setupDebugButtonDrag() {
	...
	$(document).on("mousemove.rpgDebugDrag", (e) => {
		if (!mouseDown) return;
	...
	$(document).on("mouseup.rpgDebugDrag", (e) => {
		if (mouseDown && isDragging) {
			const offset = $debugBtn.offset();
			const newPosition = {
				left: offset.left + "px",
...
export function updateFabWidgets() {
	...
	if (userStats) {
		try {
			statsData =
				typeof userStats === "object" ? userStats : JSON.parse(userStats);
		} catch (e) {
	...
	const $container = $(
		'<div id="rpg-fab-widget-container" class="rpg-fab-widget-container"></div>',
	);
	$container.css({
	...
	if (widgetSettings.attributes?.enabled) {
		...
		const showRPGAttributes =
			extensionSettings.trackerConfig?.userStats?.showRPGAttributes !== false;
		...
		if (showRPGAttributes && extensionSettings.classicStats) {
			...
			const enabledAttrIds = configuredAttrs
			...
			const attrItems = Object.entries(attrs)
				...
				.map(
					([key, value]) =>
						`<div class="rpg-fab-widget-attr-item"><span class="rpg-fab-widget-attr-name">${key.toUpperCase()}</span><span class="rpg-fab-widget-attr-value">${value}</span></div>`,
				)
				.join("");
	...
	widgets
		.filter((w) => w.type === "bottom")
		.forEach((w) => {
			const pos = 4; // South position
			usedPositions.add(pos);
			const finalHtml = w.html.replace(
				'class="rpg-fab-widget',
				`class="rpg-fab-widget rpg-fab-widget-pos-${pos}`,
			);
	...
	largeWidgets.forEach((w) => {
		...
		const finalHtml = w.html.replace(
			'class="rpg-fab-widget',
			`class="rpg-fab-widget rpg-fab-widget-pos-${pos}${centeredClass}`,
		);
		positionedWidgets.push({ position: pos, html: finalHtml });
	});
	...
	widgets
		...
		.forEach((w) => {
			...
			while (
				smallPosIndex < smallPositionOrder.length &&
				usedPositions.has(smallPositionOrder[smallPosIndex])
			) {
				smallPosIndex++;
			}
			const pos =
				smallPosIndex < smallPositionOrder.length
					? smallPositionOrder[smallPosIndex]
					: smallPosIndex % 8;
			usedPositions.add(pos);
			smallPosIndex++;
			const finalHtml = w.html.replace(
	...
	if (positionedWidgets.length > 0) {
		...
		$container
			.find(".rpg-fab-widget[data-full-text]")
		...
		$(document).on("click.fabWidgets touchstart.fabWidgets", (e) => {
...
function getStatColor(value) {
	...
	if (!lowRGB || !highRGB)
		return value > 50
			? hexToRgba(highColor, highOpacity)
			: hexToRgba(lowColor, lowOpacity);
