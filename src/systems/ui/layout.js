	...
	if (isMobile) {
		...
		if (isLeftPanel) {
			if (isOpen) {
				...
				$icon
					.removeClass("fa-chevron-down fa-chevron-up fa-chevron-right")
			...
			if (isOpen) {
				...
				$icon
					.removeClass("fa-chevron-down fa-chevron-up fa-chevron-left")
...
export function setupCollapseToggle() {
	...
	$collapseToggle.attr(
		"title",
		i18n.getTranslation("template.mainPanel.collapseExpand"),
	);
	const $panel = $("#rpg-companion-panel");
	const $icon = $collapseToggle.find("i");
...
export function updateSectionVisibility() {
	...
	const showDividerAfterThoughts =
		extensionSettings.showCharacterThoughts &&
	...
	const showDividerAfterInventory =
		extensionSettings.showInventory && extensionSettings.showQuests;
...
export function applyPanelPosition() {
	...
	$panelContainer.removeClass(
		"rpg-position-left rpg-position-right rpg-position-top",
	);
	$("body").removeClass(
		"rpg-panel-position-left rpg-panel-position-right rpg-panel-position-top",
	);
