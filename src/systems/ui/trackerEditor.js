...
export function initTrackerEditor() {
	...
	// Save button
	$(document).on("click", "#rpg-editor-save", () => {
		applyTrackerConfig();
		closeTrackerEditor();
	});

	// Cancel button
	$(document).on("click", "#rpg-editor-cancel", () => {
		closeTrackerEditor();
	});

	// Close X button
	$(document).on("click", "#rpg-close-tracker-editor", () => {
		closeTrackerEditor();
	});

	// Reset button
	$(document).on("click", "#rpg-editor-reset", () => {
		resetToDefaults();
		renderEditorUI();
	});

	// Close on background click
	$(document).on("click", "#rpg-tracker-editor-popup", (e) => {
	...
	// Open button
	$(document).on("click", "#rpg-open-tracker-editor", () => {
		openTrackerEditor();
	});

	// Export button
	$(document).on("click", "#rpg-editor-export", () => {
		exportTrackerPreset();
	});

	// Import button
	$(document).on("click", "#rpg-editor-import", () => {
		importTrackerPreset();
	});
		...
		if (presetId && presetId !== getActivePresetId()) {
			...
			if (loadPreset(presetId)) {
				...
				if (wasAssociated && entityKey) {
					...
					toastr.info(
						`"${preset?.name || "Unknown"}" will be associated with ${getCurrentEntityName()} when saved.`,
					);
				} else {
					toastr.success(
						`Switched to preset "${getPreset(presetId)?.name || "Unknown"}".`,
					);
				}
	...
	// New preset button
	$(document).on("click", "#rpg-preset-new", () => {
	...
	$(document).on("click", "#rpg-preset-default", () => {
		const currentPresetId = getActivePresetId();
		if (currentPresetId) {
			...
			toastr.success(
				`"${preset?.name || "Unknown"}" is now the default preset.`,
			);
		}
	});

	// Delete preset button
	$(document).on("click", "#rpg-preset-delete", () => {
		const currentPresetId = getActivePresetId();
		...
		if (
			confirm(
				`Are you sure you want to delete the preset "${preset?.name || "Unknown"}"?`,
			)
		) {
	...
	$(document).on("change", "#rpg-preset-associate", function () {
		...
		if ($(this).is(":checked")) {
			...
			toastr.info(
				`"${preset?.name || "Unknown"}" will be associated with ${entityName} when saved.`,
			);
		} else {
			...
			if (defaultPreset && defaultPresetId !== activePresetId) {
				toastr.info(
					`Association will be removed when saved. Default preset "${defaultPreset.name}" will apply on next character switch.`,
				);
			} else {
				toastr.info(
					`Association will be removed for ${entityName} when saved.`,
				);
			}
		}
	});
...
function updatePresetUI() {
	...
	for (const [id, preset] of Object.entries(presets)) {
		...
		$select.append(
			`<option value="${id}">${starPrefix}${preset.name}</option>`,
		);
	}
	...
	if (isDefaultPreset(activePresetId)) {
		$defaultBtn
			.addClass("rpg-btn-active")
			.attr("title", "This is the default preset");
	} else {
		$defaultBtn
			.removeClass("rpg-btn-active")
			.attr("title", "Set as Default Preset");
	}
...
function openTrackerEditor() {
	...
	originalAssociation = {
		presetId: currentAssociatedPreset,
		entityKey: entityKey,
	};
...
function applyTrackerConfig() {
	...
	if (tempAssociation) {
		if (tempAssociation.presetId !== null) {
			...
			toastr.success(
				`"${preset?.name || "Unknown"}" is now associated with ${getCurrentEntityName()}.`,
			);
		} else {
			// Remove association
			removePresetAssociationForCurrentEntity();
	...
	extensionSettings.trackerConfig = {
		userStats: {
			customStats: [
				{
					id: "health",
					name: i18n.getTranslation("stats.health"),
					enabled: true,
					persistInHistory: false,
				},
				{
					id: "satiety",
					name: i18n.getTranslation("stats.satiety"),
					enabled: true,
					persistInHistory: false,
				},
				{
					id: "energy",
					name: i18n.getTranslation("stats.energy"),
					enabled: true,
					persistInHistory: false,
				},
				{
					id: "hygiene",
					name: i18n.getTranslation("stats.hygiene"),
					enabled: true,
					persistInHistory: false,
				},
				{
					id: "arousal",
					name: i18n.getTranslation("stats.arousal"),
					enabled: true,
					persistInHistory: false,
				},
			],
			...
			rpgAttributes: [
				{
					id: "str",
					name: i18n.getTranslation("stats.str"),
					enabled: true,
					persistInHistory: false,
				},
				{
					id: "dex",
					name: i18n.getTranslation("stats.dex"),
					enabled: true,
					persistInHistory: false,
				},
				{
					id: "con",
					name: i18n.getTranslation("stats.con"),
					enabled: true,
					persistInHistory: false,
				},
				{
					id: "int",
					name: i18n.getTranslation("stats.int"),
					enabled: true,
					persistInHistory: false,
				},
				{
					id: "wis",
					name: i18n.getTranslation("stats.wis"),
					enabled: true,
					persistInHistory: false,
				},
				{
					id: "cha",
					name: i18n.getTranslation("stats.cha"),
					enabled: true,
					persistInHistory: false,
				},
			],
	...
	try {
		...
		const preset = {
			...
			historyPersistence: historyPersistence
				? JSON.parse(JSON.stringify(historyPersistence))
				: null, // Include history persistence settings
		};
		...
		toastr.success(
			i18n.getTranslation(
				"template.trackerEditorModal.messages.exportSuccess",
			) || "Tracker preset exported successfully!",
		);
	} catch (error) {
...
function migrateTrackerPreset(config) {
	...
	if (migrated.presentCharacters) {
		...
		if (migrated.presentCharacters.customFields) {
			migrated.presentCharacters.customFields =
				migrated.presentCharacters.customFields.map((field) => ({
					...field,
		...
		if (
			migrated.presentCharacters.thoughts &&
			migrated.presentCharacters.thoughts.persistInHistory === undefined
		) {
	...
	if (migrated.userStats) {
		...
		if (migrated.userStats.customStats) {
			migrated.userStats.customStats = migrated.userStats.customStats.map(
				(stat) => ({
					...stat,
		...
		if (migrated.userStats.rpgAttributes) {
			migrated.userStats.rpgAttributes = migrated.userStats.rpgAttributes.map(
				(attr) => ({
					...attr,
		...
		// Status section
		if (
			migrated.userStats.statusSection &&
			migrated.userStats.statusSection.persistInHistory === undefined
		) {
		...
		// Skills section
		if (
			migrated.userStats.skillsSection &&
			migrated.userStats.skillsSection.persistInHistory === undefined
		) {
...
function importTrackerPreset() {
	...
	input.onchange = async (e) => {
		...
		try {
			...
			if (
				!data.trackerConfig.userStats ||
				!data.trackerConfig.infoBox ||
				!data.trackerConfig.presentCharacters
			) {
				throw new Error(
					"Invalid preset file: missing required configuration sections",
				);
			}
			...
			toastr.error(
				i18n.getTranslation(
					"template.trackerEditorModal.messages.importError",
...
function renderUserStatsTab() {
	...
	const alwaysSendAttributes =
		config.alwaysSendAttributes !== undefined
			? config.alwaysSendAttributes
			: false;
	html += '<div class="rpg-editor-toggle-row">';
...
function setupUserStatsListeners() {
	...
	$(".rpg-stat-toggle")
		...
		.on("change", function () {
			const index = $(this).data("index");
			extensionSettings.trackerConfig.userStats.customStats[index].enabled =
				$(this).is(":checked");
		});
	...
	$(".rpg-stat-name")
		...
		.on("blur", function () {
			const index = $(this).data("index");
			extensionSettings.trackerConfig.userStats.customStats[index].name =
				$(this).val();
		});
	...
	$(".rpg-stat-max")
		...
		.on("blur", function () {
			...
			extensionSettings.trackerConfig.userStats.customStats[index].maxValue =
				Math.max(1, value);
		});

	// Stats display mode toggle
	$('input[name="stats-display-mode"]')
		.off("change")
		.on("change", function () {
			extensionSettings.trackerConfig.userStats.statsDisplayMode =
				$(this).val();
	...
	$("#rpg-add-attr")
		...
		.on("click", () => {
			...
			if (
				!extensionSettings.trackerConfig.userStats.rpgAttributes ||
				extensionSettings.trackerConfig.userStats.rpgAttributes.length === 0
			) {
				extensionSettings.trackerConfig.userStats.rpgAttributes = [
					{ id: "str", name: "STR", enabled: true },
					{ id: "dex", name: "DEX", enabled: true },
	...
	$(".rpg-attr-toggle")
		...
		.on("change", function () {
			const index = $(this).data("index");
			extensionSettings.trackerConfig.userStats.rpgAttributes[index].enabled =
				$(this).is(":checked");
		});
	...
	$(".rpg-attr-name")
		...
		.on("blur", function () {
			const index = $(this).data("index");
			extensionSettings.trackerConfig.userStats.rpgAttributes[index].name =
				$(this).val();
		});
	...
	$("#rpg-show-level")
		.off("change")
		.on("change", function () {
			extensionSettings.trackerConfig.userStats.showLevel =
				$(this).is(":checked");
		});

	// Always send attributes toggle
	$("#rpg-always-send-attrs")
		.off("change")
		.on("change", function () {
			extensionSettings.trackerConfig.userStats.alwaysSendAttributes =
				$(this).is(":checked");
		});
	...
	$("#rpg-status-enabled")
		.off("change")
		.on("change", function () {
			extensionSettings.trackerConfig.userStats.statusSection.enabled =
				$(this).is(":checked");
		});
	...
	$("#rpg-mood-emoji")
		.off("change")
		.on("change", function () {
			extensionSettings.trackerConfig.userStats.statusSection.showMoodEmoji =
				$(this).is(":checked");
		});
	...
	$("#rpg-skills-enabled")
		.off("change")
		.on("change", function () {
			extensionSettings.trackerConfig.userStats.skillsSection.enabled =
				$(this).is(":checked");
		});
	...
	$("#rpg-skills-label")
		.off("blur")
		.on("blur", function () {
			extensionSettings.trackerConfig.userStats.skillsSection.label =
				$(this).val();
			saveSettings();
		});
...
function setupPresentCharactersListeners() {
	...
	$("#rpg-add-relationship")
		...
		.on("click", () => {
			...
			const baseName = "New Relationship";
	...
	$(".rpg-relationship-emoji")
		...
		.on("blur", function () {
			const name = $(this)
				.closest(".rpg-relationship-item")
				.find(".rpg-relationship-name")
				.val();
	...
	$("#rpg-thoughts-name")
		...
		.on("blur", function () {
			...
			extensionSettings.trackerConfig.presentCharacters.thoughts.name =
				$(this).val();
		});

	$("#rpg-thoughts-description")
		.off("blur")
		.on("blur", function () {
			if (!extensionSettings.trackerConfig.presentCharacters.thoughts) {
				extensionSettings.trackerConfig.presentCharacters.thoughts = {};
			}
			extensionSettings.trackerConfig.presentCharacters.thoughts.description =
				$(this).val();
		});
	...
	$(".rpg-field-remove")
		...
		.on("click", function () {
			const index = $(this).data("index");
			extensionSettings.trackerConfig.presentCharacters.customFields.splice(
				index,
				1,
			);
			renderPresentCharactersTab();
	...
	$("#rpg-char-stats-enabled")
		...
		.on("change", function () {
			...
			extensionSettings.trackerConfig.presentCharacters.characterStats.enabled =
				$(this).is(":checked");
		});
	...
	$(".rpg-char-stat-remove")
		.off("click")
		.on("click", function () {
			const index = $(this).data("index");
			extensionSettings.trackerConfig.presentCharacters.characterStats.customStats.splice(
				index,
				1,
			);
			renderPresentCharactersTab();
...
function setupHistoryPersistenceListeners() {
	...
	// Main toggle
	$("#rpg-history-persistence-enabled")
		.off("change")
		.on("change", function () {
	...
	$("#rpg-history-send-all-enabled")
		.off("change")
		.on("change", function () {
			extensionSettings.historyPersistence.sendAllEnabledOnRefresh =
				$(this).is(":checked");
		});

	// Message count
	$("#rpg-history-message-count")
		.off("change")
		.on("change", function () {
			extensionSettings.historyPersistence.messageCount =
				parseInt($(this).val()) || 0;
		});

	// Injection position
	$("#rpg-history-injection-position")
		.off("change")
		.on("change", function () {
	...
	// User Stats toggles
	$(".rpg-history-stat-toggle")
		.off("change")
		.on("change", function () {
			const index = $(this).data("index");
			extensionSettings.trackerConfig.userStats.customStats[
				index
			].persistInHistory = $(this).is(":checked");
		});
	...
	$("#rpg-history-status")
		.off("change")
		.on("change", function () {
			extensionSettings.trackerConfig.userStats.statusSection.persistInHistory =
				$(this).is(":checked");
		});
	...
	$("#rpg-history-skills")
		.off("change")
		.on("change", function () {
			extensionSettings.trackerConfig.userStats.skillsSection.persistInHistory =
				$(this).is(":checked");
		});

	// Inventory
	$("#rpg-history-inventory")
		.off("change")
		.on("change", function () {
			extensionSettings.trackerConfig.userStats.inventoryPersistInHistory =
				$(this).is(":checked");
		});
	...
	$("#rpg-history-quests")
		.off("change")
		.on("change", function () {
			extensionSettings.trackerConfig.userStats.questsPersistInHistory =
				$(this).is(":checked");
		});

	// Info Box widget toggles
	$(".rpg-history-widget-toggle")
		.off("change")
		.on("change", function () {
			const widgetId = $(this).data("widget");
			extensionSettings.trackerConfig.infoBox.widgets[
				widgetId
			].persistInHistory = $(this).is(":checked");
		});

	// Present Characters field toggles
	$(".rpg-history-charfield-toggle")
		.off("change")
		.on("change", function () {
			const index = $(this).data("index");
			extensionSettings.trackerConfig.presentCharacters.customFields[
