...
function setLockedItemsInSwipeStore(trackerType, lockedItems) {
	if (isGenerating) {
		console.warn(
			"[RPG Lock Manager] Attempted to set locked items while generation is in progress.",
		);
		return;
	}
...
function applyUserStatsLocks(data, lockedItems) {
	...
	if (data.inventory && lockedItems.inventory) {
		...
		if (data.inventory.onPerson) {
			data.inventory.onPerson = applyInventoryLocks(
				data.inventory.onPerson,
				"onPerson",
			);
		}
		...
		if (data.inventory.clothing) {
			data.inventory.clothing = applyInventoryLocks(
				data.inventory.clothing,
				"clothing",
			);
		}
		...
		if (data.inventory.assets) {
			data.inventory.assets = applyInventoryLocks(
		...
		if (data.inventory.stored && lockedItems.inventory.stored) {
			for (const location in data.inventory.stored) {
				if (
					Array.isArray(data.inventory.stored[location]) &&
					lockedItems.inventory.stored[location]
				) {
					data.inventory.stored[location] = data.inventory.stored[location].map(
						(item) => {
							const itemName = item?.name || "";
...
function applyCharactersLocks(data, lockedItems) {
				...
				if (charLocks[fieldName] === true) {
					...
					if (!locked && modifiedChar.details) {
						if (modifiedChar.details[fieldName] !== undefined) {
							...
							if (
								...
								typeof modifiedChar.details !== "object"
							) {
								modifiedChar.details = {};
							} else {
					...
					if (!locked && modifiedChar.relationship) {
						if (modifiedChar.relationship[fieldName] !== undefined) {
							...
							modifiedChar.relationship[snakeCaseFieldName] !== undefined
						) {
					...
					if (!locked && modifiedChar.thoughts) {
						if (modifiedChar.thoughts[fieldName] !== undefined) {
							...
							modifiedChar.thoughts[snakeCaseFieldName] !== undefined
						) {
	...
	const result = Array.isArray(data) ? characters : { ...data, characters };
...
export function setItemLock(trackerType, itemPath, locked) {
	...
	console.log(
		"[RPG Lock Manager] Locked items after set:",
		JSON.stringify(lockedItems, null, 2),
	);
}
...
export function restoreLockedContent(trackerData, previousData, trackerType) {
	if (!trackerData || !previousData) {
		console.log(
			"[RPG Lock Manager] restoreLockedContent: Missing data, skipping restoration",
		);
		return trackerData;
	}
	...
	if (lockedItems.stats && lockedItems.stats === true && prevData.stats) {
		...
		for (const prevStat of prevData.stats) {
			if (!existingIds.includes(prevStat.id)) {
				console.warn(
					"[RPG Lock Manager] Stat missing, restoring:",
					prevStat.id,
				);
				...
				const newStatIndex = result.stats.findIndex(
					(s) => s.id === prevStat.id,
				);
		...
		for (const skillName in lockedItems.skills) {
			if (lockedItems.skills[skillName]) {
				...
				if (prevSkillIndex !== -1) {
					if (newSkillIndex === -1) {
						console.log(
							"[RPG Lock Manager] Skill",
							skillName,
							"is missing, restoring from previous",
						);
						...
						if (
							typeof prevData.skills[prevSkillIndex] === "object" &&
							typeof result.skills[newSkillIndex] === "object"
						) {
							result.skills[newSkillIndex] = {
								...prevData.skills[prevSkillIndex],
							};
						}
					}
				}
			...
			if (lockedCategory && prevCategory && Array.isArray(prevCategory)) {
				for (const itemName in lockedCategory) {
					if (lockedCategory[itemName]) {
						...
						const newItemIndex = result.inventory[category].findIndex(
							(item) => {
								const name =
									typeof item === "string"
										? item
						...
						if (prevItemIndex !== -1) {
							if (newItemIndex === -1) {
								console.log(
									"[RPG Lock Manager] Inventory item",
									itemName,
									"is missing, restoring from previous",
								);
					...
					if (lockedLocation && prevLocation && Array.isArray(prevLocation)) {
						for (const itemName in lockedLocation) {
							if (lockedLocation[itemName]) {
								...
								const newItemIndex = result.inventory.stored[
									location
								].findIndex((item) => {
									const name =
										typeof item === "string"
								...
								if (prevItemIndex !== -1 && newItemIndex === -1) {
									...
									result.inventory.stored[location].push(
										prevLocation[prevItemIndex],
									);
								}
							}
						}
	...
	if (lockedItems.quests && prevData.quests) {
		...
		if (lockedItems.quests.main && prevData.quests.main) {
			...
			if (prevData.quests.main) {
				console.log(
					"[RPG Lock Manager] Main quest has changed, restoring from previous",
				);
		...
		if (
			lockedItems.quests.optional &&
			Array.isArray(prevData.quests.optional)
		) {
			if (!result.quests) {
				result.quests = {};
			}
				...
				if (key.startsWith("optional[") && lockedItems.quests[key]) {
					...
					if (indexMatch) {
						...
						if (index < prevData.quests.optional.length) {
							...
							console.log(
								"[RPG Lock Manager] Checking optional quest at index:",
								index,
								"quest:",
								prevQuest,
							);
							...
							if (index < result.quests.optional.length) {
								console.log(
									"[RPG Lock Manager] Optional quest at index",
									index,
									"exists but may be modified, restoring from previous",
								);
								...
								console.log(
									"[RPG Lock Manager] Optional quest at index",
									index,
									"is missing, appending from previous",
								);
...
function restoreCharacters(newData, prevData, lockedItems) {
	...
	// Handle both array format and object format
	let charactersNew = Array.isArray(newData)
		? newData
		: newData.characters || [];
	const charactersPrev = Array.isArray(prevData)
		? prevData
		: prevData.characters || [];

	console.log("[RPG Lock Manager] New characters count:", charactersNew.length);
	console.log(
		"[RPG Lock Manager] Previous characters count:",
		charactersPrev.length,
	);
	...
	for (const key in lockedItems) {
		if (
			...
			(typeof lockedItems[key] === "object" &&
				Object.keys(lockedItems[key]).length > 0)
		) {
			lockedCharNames.add(key);
		}
	}
	console.log(
		"[RPG Lock Manager] Locked character names:",
		Array.from(lockedCharNames),
	);
	...
	charactersNew = charactersNew.map((char) => {
		...
		const prevChar = charactersPrev.find(
			(c) => (c.name || c.characterName) === charName,
		);
		if (!prevChar) {
			console.warn(
				"[RPG Lock Manager] Previous character data not found for:",
				charName,
				"cannot restore locked fields",
			);
			return resultChar;
		}
		...
		for (const fieldName in charLocks) {
			if (charLocks[fieldName] === true) {
				...
				if (prevChar[fieldName] !== undefined) {
					...
					prevChar.details &&
					prevChar.details[fieldName] !== undefined
				) {
					resultChar.details[fieldName] = prevChar.details[fieldName];
				} else if (
					prevChar.relationship &&
					prevChar.relationship[fieldName] !== undefined
				) {
					resultChar.relationship[fieldName] = prevChar.relationship[fieldName];
				} else if (
					prevChar.thoughts &&
					prevChar.thoughts[fieldName] !== undefined
				) {
 ...
 */
export function removeInventoryItemLock(
	trackerType,
	field,
	itemName,
	location,
	...
	if (field === "onPerson") {
		if (
			lockedItems.inventory.onPerson &&
			lockedItems.inventory.onPerson[itemName]
		) {
			delete lockedItems.inventory.onPerson[itemName];
			console.log(
				"[RPG Lock Manager] Removed lock for onPerson item:",
				itemName,
			);
		}
	} else if (field === "assets") {
		if (
			lockedItems.inventory.assets &&
			lockedItems.inventory.assets[itemName]
		) {
		...
		if (
			lockedItems.inventory.stored &&
			lockedItems.inventory.stored[location] &&
			lockedItems.inventory.stored[location][itemName]
		) {
			delete lockedItems.inventory.stored[location][itemName];
			console.log(
				"[RPG Lock Manager] Removed lock for stored item:",
				itemName,
				"at location:",
				location,
			);
