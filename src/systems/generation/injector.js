...
function buildHistoricalContextMap() {
	...
	const maxMessages =
		messageCount === 0 ? chat.length : Math.min(messageCount, chat.length);
	...
	for (
		let i = startIndex;
		i >= 0 && (messageCount === 0 || processedCount < maxMessages);
		i--
	) {
...
function injectContextIntoTextPrompt(prompt) {
	...
	const sortedEntries = Array.from(pendingContextMap.entries()).sort(
		(a, b) => b[0] - a[0],
	);
	...
	for (const [msgIdx, ctxContent] of sortedEntries) {
		...
		if (!position) {
			...
			console.debug(
				`[RPG Companion] Could not find message ${msgIdx} in prompt for context injection`,
			);
			continue;
		}
		...
		modifiedPrompt =
			modifiedPrompt.slice(0, position.end) +
			ctxContent +
			modifiedPrompt.slice(position.end);
		injectedCount++;
	}

	if (injectedCount > 0) {
		console.log(
			`[RPG Companion] Injected historical context into ${injectedCount} positions in text prompt`,
		);
	}
...
function injectContextIntoChatPrompt(chatMessages) {
	...
	for (const [msgIdx, ctxContent] of pendingContextMap) {
		...
		if (!found) {
			console.debug(
				`[RPG Companion] Could not find message ${msgIdx} in chat prompt for context injection`,
			);
		}
	}
	...
	if (injectedCount > 0) {
		console.log(
			`[RPG Companion] Injected historical context into ${injectedCount} messages in chat prompt`,
		);
	}
...
function injectContextIntoFinalMesSend(finalMesSend) {
	if (
		pendingContextMap.size === 0 ||
		!Array.isArray(finalMesSend) ||
		finalMesSend.length === 0
	) {
		return 0;
	}
	...
	for (
		let chatIdx = 0;
		chatIdx < chat.length && mesSendIdx < finalMesSend.length;
		chatIdx++
	) {
		...
		while (mesSendIdx < finalMesSend.length) {
			...
			const matchContent =
				chatContent.length > 50 ? chatContent.substring(0, 50) : chatContent;
	...
	for (const [chatIdx, ctxContent] of pendingContextMap) {
		...
		if (targetMesSendIdx === undefined) {
			console.debug(
				`[RPG Companion] Chat message ${chatIdx} not found in finalMesSend mapping`,
			);
			continue;
		}
		...
		injectedCount++;
		console.debug(
			`[RPG Companion] Injected context for chat[${chatIdx}] into finalMesSend[${targetMesSendIdx}]`,
		);
	}
...
function onGenerateBeforeCombinePrompts(eventData) {
	...
	if (injectedCount > 0) {
		console.log(
			`[RPG Companion] Injected historical context into ${injectedCount} messages in finalMesSend`,
		);
...
function onGenerateAfterCombinePrompts(eventData) {
	...
	if (!historyInjectionDone && pendingContextMap.size > 0) {
		...
		console.log(
			"[RPG Companion] Using fallback string-based injection (AFTER_COMBINE)",
		);
...
function validateSettings(settings) {
	if (!settings || typeof settings !== "object") {
		console.error(
			"[RPG Companion] Invalid extensionSettings: must be an object",
		);
		return false;
	}
	...
	if (missingKeys.length > 0) {
		console.warn(
			`[RPG Companion] Missing required settings: ${missingKeys.join(", ")}`,
		);
		return false;
	}
 ...
 */
export async function onGenerationStarted(_type, data, dryRun) {
	// Skip dry runs (page reload, prompt manager preview, etc.)
	if (dryRun) {
		console.debug(
			"[RPG Companion] Skipping onGenerationStarted: dry run detected",
		);
		return;
	}
	...
	if (!validateSettings(extensionSettings)) {
		console.error(
			"[RPG Companion] Invalid settings detected, aborting generation",
		);
		return;
	}
	...
	if (data?.quietImage || data?.quiet_image || data?.isImageGeneration) {
		console.debug(
			"[RPG Companion] Detected image generation, skipping tracker injection",
		);
		return;
	}
	...
	if (!extensionSettings.enabled) {
		...
		setExtensionPrompt(
			...
			INJECTION_DEPTHS.FIRST_MESSAGE,
			false,
		);
		setExtensionPrompt(
		...
		setExtensionPrompt(
			...
			"",
			extension_prompt_types.IN_CHAT,
			INJECTION_DEPTHS.BEFORE_LAST_MESSAGE,
			false,
		);
		return;
	}
	...
	if (shouldSuppress) {
		console.debug(
			`[RPG Companion] Suppression active (mode=${skipMode}). isGuided=${isGuidedGeneration}, isImpersonation=${isImpersonationGeneration}, hasQuietPrompt=${hasQuietPrompt} - skipping RPG tracker injections for this generation.`,
		);
	...
	// In SINGLE mode, inject the contextual summary for main roleplay generation
	const contextSummary = generateContextualSummary();

	if (contextSummary) {
		// Use custom context instructions prompt if set, otherwise use default
		const contextInstructionsText =
		...
		console.debug("[RPG Companion] Injected contextual summary");
	} else {
...
export function initHistoryInjectionListeners() {
	...
	if (!eventSource || typeof eventSource.on !== "function") {
		console.error(
			"[RPG Companion] Event source not available, cannot initialize listeners",
		);
		return;
	}
