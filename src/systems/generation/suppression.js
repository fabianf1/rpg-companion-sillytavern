...
export function evaluateSuppression(extensionSettings, context, data) {
	...
	const IMPERSONATION_PATTERNS = [
		...
		{
			id: "second-perspective",
			re: /write in second person perspective from/i,
		},
	...
	const combinedTextForDetection = [instructContent, quietPromptRaw]
		.filter(Boolean)
		.join("\n");
	...
	const skipMode =
		(extensionSettings && extensionSettings.skipInjectionsForGuided) || "none";

	// Compute suppression according to mode
	const shouldSuppress =
		skipMode === "guided"
			? isGuidedGeneration || hasQuietPrompt
			: skipMode === "impersonation"
